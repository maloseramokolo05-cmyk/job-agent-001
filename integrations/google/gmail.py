...
def build_message(to,subject,body,reply_to=None,attachments=None):
 ...
 for path in attachments or []:
  path=Path(path);mime=mimetypes.guess_type(path.name)[0] or "application/octet-stream";major,minor=mime.split("/",1);msg.add_attachment(path.read_bytes(),maintype=major,subtype=minor,filename=path.name)
 return base64.urlsafe_b64encode(msg.as_bytes()).decode()
def create_draft(to,subject,body,thread_id=None,http=requests,attachments=None):
 token=access_token(http); payload={"message":{"raw":build_message(to,subject,body,attachments=attachments)}}
...
def send_message(to,subject,body,explicit=False,http=requests,attachments=None):
 ...
 token=access_token(http); response=http.post(f"{BASE}/messages/send",headers={**_headers(token),"Content-Type":"application/json"},json={"raw":build_message(to,subject,body,attachments=attachments)},timeout=30); response.raise_for_status(); return response.json()
