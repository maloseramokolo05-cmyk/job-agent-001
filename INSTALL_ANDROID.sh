...
if ! grep -Eq '^ADMIN_PASSWORD=.{12,}$' .env; then
  PASSWORD="$(python -c 'import secrets; print(secrets.token_urlsafe(18))')"
  sed -i "s|^ADMIN_PASSWORD=.*|ADMIN_PASSWORD=$PASSWORD|" .env
  echo "Generated owner password (save it securely): $PASSWORD"
fi
