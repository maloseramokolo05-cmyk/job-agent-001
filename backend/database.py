...
CREATE TABLE IF NOT EXISTS sessions(id INTEGER PRIMARY KEY, token_hash TEXT UNIQUE NOT NULL, user_id INTEGER NOT NULL, csrf_token TEXT NOT NULL, expires_at TEXT NOT NULL, created_at TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS job_sources_found(id INTEGER PRIMARY KEY, job_id INTEGER NOT NULL, source TEXT NOT NULL, source_url TEXT NOT NULL, discovered_at TEXT NOT NULL, UNIQUE(job_id,source_url), FOREIGN KEY(job_id) REFERENCES jobs(id));
CREATE TABLE IF NOT EXISTS run_locks(lock_name TEXT PRIMARY KEY, owner TEXT NOT NULL, expires_at TEXT NOT NULL);
...
def migrate():
 with connect() as db:
  ...
  _column(db,"jobs","employment_type","TEXT DEFAULT ''")
  _column(db,"jobs","structured_metadata","TEXT DEFAULT '{}'")
  _column(db,"jobs","verification_status","TEXT DEFAULT 'UNVERIFIED'")
  _column(db,"source_status","jobs_found","INTEGER DEFAULT 0")
  ...
  db.execute("UPDATE runs SET state='INTERRUPTED', finished_at=? WHERE state='RUNNING' AND julianday(started_at)<julianday('now','-5 minutes')",(now(),))
def init_db(): migrate()
def schema_version():
 with connect() as db:
