...
import base64,csv,io,json,logging,os,platform,secrets
...
from backend.config import ROOT,DATA_ROOT,load_profile,load_preferences,save_json,database_path,storage_path
...
def _auth_required():return os.getenv("APP_AUTH_ENABLED","false").lower() in {"1","true","yes","on"}
def _authorized(request:Request):
 if not _auth_required():return True
 expected_user=os.getenv("APP_USERNAME","");expected_password=os.getenv("APP_PASSWORD","")
 if not expected_user or not expected_password:return False
 header=request.headers.get("authorization","")
 if not header.lower().startswith("basic "):return False
 try:
  raw=base64.b64decode(header.split(" ",1)[1]).decode("utf-8");user,password=raw.split(":",1)
 except Exception:return False
 return secrets.compare_digest(user,expected_user) and secrets.compare_digest(password,expected_password)

@app.middleware("http")
async def private_workspace(request:Request,call_next):
 if request.url.path=="/api/health" or _authorized(request):return await call_next(request)
 if _auth_required() and (not os.getenv("APP_USERNAME") or not os.getenv("APP_PASSWORD")):
  return PlainTextResponse("Hosted authentication is enabled but credentials are not configured.",status_code=503)
 return PlainTextResponse("Authentication required",status_code=401,headers={"WWW-Authenticate":"Basic realm=\"Tumelo Job Agent\""})

def setup_logging():
 log=storage_path("logs","job_agent.log");handler=RotatingFileHandler(log,maxBytes=2_000_000,backupCount=4,encoding="utf-8");handler.setFormatter(logging.Formatter('{"time":"%(asctime)s","severity":"%(levelname)s","component":"%(name)s","message":"%(message)s"}'));root_logger=logging.getLogger();root_logger.setLevel(logging.INFO)
 if not any(isinstance(existing,RotatingFileHandler) and getattr(existing,"baseFilename",None)==str(log) for existing in root_logger.handlers):root_logger.addHandler(handler)

@app.on_event("startup")
def startup():init_db();setup_logging();logging.getLogger("startup").info("event=APPLICATION_STARTED version=%s",__version__)
@app.get("/api/health")
def health():
 google=TokenStore().summary();return {"status":"ok","version":__version__,"platform":platform.system().lower(),"hosted":bool(os.getenv("RENDER") or os.getenv("APP_BASE_URL")),"database":{"ready":schema_version()==2,"schema_version":schema_version()},"scheduler":"external_process","google":{"connected":google["connected"]},"job_connector_count":1}
@app.get("/api/config")
...
async def upload_cv(file:UploadFile=File(...)):
 ...
 path=storage_path("cv","master_cv"+suffix);path.write_bytes(content);return {"saved":str(path.relative_to(DATA_ROOT))}
@app.get("/api/jobs")
...
def document_download(document_id:int):
 ...
 relative=Path(doc["local_path"])
 if relative.is_absolute() or ".." in relative.parts:raise HTTPException(400,"Invalid document path")
 path=storage_path(*relative.parts)
 if not path.exists():raise HTTPException(404,"Document file is unavailable")
...
def drive_upload(document_id:int):
 ...
 relative=Path(doc["local_path"])
 if relative.is_absolute() or ".." in relative.parts:raise HTTPException(400,"Invalid document path")
 result=upload_file(storage_path(*relative.parts),existing_id=doc.get("drive_file_id"))
...
@app.get("/api/logs")
def logs():
 path=storage_path("logs","job_agent.log");lines=path.read_text(encoding="utf-8",errors="replace").splitlines()[-200:] if path.exists() else [];return {"lines":lines}
