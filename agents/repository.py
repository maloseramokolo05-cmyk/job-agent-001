...
def ingest(job):
 ...
 try:
  with connect() as db:
   cur=db.execute("INSERT INTO jobs(vacancy_id,title,normalized_title,company,normalized_company,location,salary,date_posted,closing_date,description,requirements,source,vacancy_url,application_url,work_mode,discovered_at,description_hash,status) VALUES("+",".join("?"*18)+")",values+("DISCOVERED",)); return cur.lastrowid,False
