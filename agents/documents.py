...
GENERATOR_VERSION="3.0.0"; TEMPLATE_VERSION="ats-mobile-1"
...
def record(job_id,kind,path,master):
 digest=hashlib.sha256(master.encode()).hexdigest(); content=hashlib.sha256((DATA_ROOT/path).read_bytes()).hexdigest()
...
def generate_cv(job,profile,cv_text,job_id=None):
 ...
 stem=f"{date.today().isoformat()}_{safe(job['company'])}_{safe(job['title'])}_{safe(profile['name'])}_CV";out=DATA_ROOT/"generated_cvs";out.mkdir(exist_ok=True);path=out/f"{stem}.docx";doc=Document();style=doc.styles["Normal"];style.font.name="Arial";style.font.size=Pt(10);doc.add_heading(profile["name"],0);contacts=" | ".join(x for x in [profile.get("email"),profile.get("phone"),profile.get("location"),profile.get("linkedin")] if x);doc.add_paragraph(contacts);doc.add_heading("Professional Profile",1);doc.add_paragraph(f"{profile['name']} offers background relevant to {job['title']}, with emphasis on the verified experience and skills most aligned to {job['company']}’s stated requirements.");doc.add_heading("Relevant Skills and Experience",1)
 ...
 doc.save(path);relative=str(path.relative_to(DATA_ROOT));pdf=None
 try:
  ...
  canvas.save();pdf=str(pdfpath.relative_to(DATA_ROOT))
