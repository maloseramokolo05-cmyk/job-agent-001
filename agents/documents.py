...
from backend.config import DATA_ROOT,storage_path
...
def _relative(path): return str(path.relative_to(DATA_ROOT))
def record(job_id,kind,path,master):
 resolved=storage_path(*path.split("/"));digest=hashlib.sha256(master.encode()).hexdigest();content=hashlib.sha256(resolved.read_bytes()).hexdigest()
...
def generate_cv(job,profile,cv_text,job_id=None):
 ...
 doc.save(path);relative=_relative(path);pdf=None
 try:
  ...
  canvas.save();pdf=_relative(pdfpath)
