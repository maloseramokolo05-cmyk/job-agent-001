...
function updateAppMode(){
  $('#appMode').textContent=isStandalone()?'Installed app mode':'Website mode · you can optionally install it to your home screen';
}
...
async function installApp(){
  ...
  installPrompt.prompt();await installPrompt.userChoice.catch(()=>null);installPrompt=null;setInstallButtons(false);
}
window.addEventListener('beforeinstallprompt',event=>{event.preventDefault();installPrompt=event;setInstallButtons(true);});
...
$('#installApp').onclick=installApp;$('#installAppCard').onclick=installApp;
...
async function load(){
  try{
    ...
    $('#documentList').innerHTML=docs.length?docs.map(d=>`<p><b>${esc(d.document_type)}</b><br><small>${esc(d.local_path)} · ${esc(d.created_at)}</small><br><a class="button" href="/api/documents/${Number(d.id)}/download">Download</a></p>`).join(''):'<p>No generated documents yet.</p>';
    ...
    const c=await api('/api/config');$('#scheduleList').textContent=(c.preferences.schedule_times||[]).join(' · ');
    const logs=await api('/api/logs');$('#logLines').textContent=logs.lines.join('\n')||'No logs yet.';
  }catch(error){console.warn('Dashboard refresh failed',error);}
}

async function detail(id){
  const j=await api('/api/jobs/'+id);const target=safeHttpUrl(j.application_url||j.vacancy_url);
...
document.addEventListener('click',event=>{
  const review=event.target.closest('.review-job');if(review){const card=review.closest('[data-job-id]');detail(Number(card.dataset.jobId)).catch(e=>alert(e.message));return;}
  ...
  if(kind==='cv'||kind==='cover-letter')action(id,kind);else if(kind==='applied')setStatus(id,'APPLIED').catch(e=>alert(e.message));else if(kind==='open'){const url=safeHttpUrl(button.dataset.url);if(url)window.open(url,'_blank','noopener,noreferrer');}
});

$('#run').onclick=async()=>{try{await api('/api/runs',{method:'POST'});$('#progress').classList.remove('hidden');poll();}catch(e){alert(e.message);}};
async function poll(){try{const r=await api('/api/runs/latest');$('#runmsg').textContent=r.message;$('#runstate').textContent=r.state;if(r.state==='RUNNING')setTimeout(poll,1200);else{setTimeout(()=>$('#progress').classList.add('hidden'),2500);load();}}catch{setTimeout(poll,2500);}}
$('#search').oninput=load;$('#status').onchange=load;
...
$('#setup').onclick=async()=>{const c=await api('/api/config'),f=$('#form'),p=c.profile,s=c.preferences;for(const n of ['name','email','phone','location','work_authorization'])if(f[n])f[n].value=p[n]||'';f.skills.value=(p.skills||[]).join(', ');f.categories.value=(s.job_categories||[]).join(', ');f.locations.value=(s.priority_locations||[]).join(', ');f.salary.value=p.salary_expectations||'';f.score.value=s.minimum_score||75;f.schedule.value=(s.schedule_times||[]).join(', ');f.mode.value=s.application_mode||'PREPARE';$('#wizard').showModal();};
$('#form').onsubmit=async event=>{event.preventDefault();const f=event.target,c=await api('/api/config'),p=c.profile,s=c.preferences;for(const n of ['name','email','phone','location','work_authorization'])p[n]=f[n].value;p.skills=f.skills.value.split(',').map(x=>x.trim()).filter(Boolean);p.salary_expectations=f.salary.value;s.job_categories=f.categories.value.split(',').map(x=>x.trim()).filter(Boolean);s.priority_locations=f.locations.value.split(',').map(x=>x.trim()).filter(Boolean);s.minimum_score=+f.score.value;s.schedule_times=f.schedule.value.split(',').map(x=>x.trim()).filter(Boolean);s.application_mode=f.mode.value;await api('/api/setup',json('POST',{profile:p,preferences:s}));if(f.cv.files[0]){const data=new FormData();data.append('file',f.cv.files[0]);await api('/api/cv',{method:'POST',body:data});}$('#wizard').close();alert('Setup saved. Run your first search when ready.');load();};

async function registerPwa(){if(!('serviceWorker' in navigator))return;try{const registration=await navigator.serviceWorker.register('/assets/sw.js',{scope:'/assets/'});registration.update().catch(()=>null);}catch(error){console.warn('Service worker registration failed',error);}}
updateAppMode();setInstallButtons(false);api('/api/health').then(h=>{document.title=`Tumelo Job Agent ${h.version}`;if(!location.hash)location.hash='dashboard';}).catch(()=>offlineBanner.classList.remove('hidden'));registerPwa();load();
