...
const metrics = {
  jobs_found: 'Jobs discovered',
  high_match: 'High matches',
  prepared: 'Prepared',
  submitted: 'Submitted',
  needs_input: 'Needs action',
...
async function load() {
  try {
    ...
    $('#metrics').innerHTML = Object.entries(metrics).map(([k, v]) =>
      `<div class=metric><b>${esc(m[k] || 0)}</b><span>${v}</span></div>`
    ).join('');
    ...
    $('#jobcards').innerHTML = jobs.map(j =>
      `<article class=card><div class=cardtop><div class=score>${Math.round(j.score || 0)}</div><div><div class=title>${esc(j.title)}</div><div class=sub>${esc(j.company)} · ${esc(j.location || 'Location unstated')}</div></div></div><span class=pill>${esc(j.status)}</span>${j.email_verified ? '<small>✓ Verified email application route</small>' : ''}<button onclick="detail(${j.id})">Review</button></article>`
    ).join('');

    const apps = await api('/api/applications');
    const docs = await api('/api/documents');
    $('#applicationList').innerHTML = apps.length
      ? apps.map(a => `<p><b>${esc(a.title)}</b> · ${esc(a.company)}<br><small>${esc(a.status)} · ${esc(a.application_date || a.created_at)}${a.recruiter_email ? ' · ' + esc(a.recruiter_email) : ''}</small></p>`).join('')
      : '<p>No applications tracked yet.</p>';
    $('#documentList').innerHTML = docs.length
      ? docs.map(d => `<p><b>${esc(d.document_type)}</b><br><small>${esc(d.created_at)}</small><br><a class="button" href="/api/documents/${d.id}/download">Download</a></p>`).join('')
      : '<p>No generated documents yet.</p>';

    const cv = await api('/api/cv');
    $('#masterCvState').innerHTML = cv.available
      ? `<p><b>Master CV v${esc(cv.master_cv.version)}</b><br><small>${esc(cv.master_cv.original_name)}</small></p><a class="button" href="/api/cv/download">Download master CV</a>`
      : '<p><b>No master CV uploaded</b></p>';
...
async function detail(id) {
  ...
  activeJob = j;
  const email = j.email_verified && j.application_email
    ? `<h3>Verified email route</h3><p>${esc(j.application_email)}</p><div class="actions"><button onclick="emailPreview(${id})">Review email</button><button class="primary" onclick="emailApply(${id})">Send application email + PDF CV</button></div><div id="emailReview"></div>`
    : '';
...
async function emailPreview(id) {
  try {
    ...
    box.innerHTML = p.eligible
      ? `<h3>Email preview</h3><p><b>To:</b> ${esc(p.recipient)}<br><b>Subject:</b> ${esc(p.subject)}<br><b>Attachment:</b> ${esc(p.cv_filename)}</p><pre>${esc(p.body)}</pre>`
      : `<p><b>Send blocked:</b> ${esc(p.reason)}</p>`;
  } catch (e) {
    alert(e.message);
  }
}
...
function openApplication() {
  if (activeJob) window.open(activeJob.application_url || activeJob.vacancy_url, '_blank', 'noopener');
}
...
async function action(id, name) {
  try {
    await api(`/api/jobs/${id}/${name}`, { method: 'POST' });
    await detail(id);
    load();
  } catch (e) {
    alert(e.message);
...
async function setStatus(id, status) {
  await api(`/api/jobs/${id}/status`, json('POST', { status }));
  await detail(id);
  load();
}
...
async function poll() {
  ...
  $('#runmsg').textContent = r.message;
  $('#runstate').textContent = r.state;
...
$('#connectGoogle').onclick = async () => {
  try {
    const r = await api('/api/google/connect', json('POST', { features: ['gmail', 'drive', 'calendar'] }));
...
$('#syncGmail').onclick = async () => {
  try {
    const r = await api('/api/gmail/sync', { method: 'POST' });
    alert(`${r.messages_saved} job-related messages saved`);
    load();
  } catch (e) {
    alert(e.message);
  }
...
$('#disconnectGoogle').onclick = async () => {
  if (confirm('Disconnect Google from this Job Agent?')) {
...
$('#setup').onclick = async () => {
  ...
  const f = $('#form');
  const p = c.profile;
  const s = c.preferences;
  for (const n of ['name', 'email', 'phone', 'location', 'work_authorization']) if (f[n]) f[n].value = p[n] || '';
  f.skills.value = (p.skills || []).join(', ');
  f.categories.value = (s.job_categories || []).join(', ');
  f.locations.value = (s.priority_locations || []).join(', ');
  f.salary.value = p.salary_expectations || '';
  f.score.value = s.minimum_score || 68;
  f.schedule.value = (s.schedule_times || []).join(', ');
  f.mode.value = s.application_mode || 'AUTO_EMAIL';
  $('#wizard').showModal();
};
...
$('#form').onsubmit = async e => {
  ...
  p.salary_expectations = f.salary.value;
  s.job_categories = f.categories.value.split(',').map(x => x.trim()).filter(Boolean);
  s.priority_locations = f.locations.value.split(',').map(x => x.trim()).filter(Boolean);
  s.minimum_score = +f.score.value;
  s.schedule_times = f.schedule.value.split(',').map(x => x.trim()).filter(Boolean);
  s.application_mode = f.mode.value;
  await api('/api/setup', json('POST', { profile: p, preferences: s }));
  if (f.cv.files[0]) {
    const data = new FormData();
    data.append('file', f.cv.files[0]);
    await api('/api/cv', { method: 'POST', body: data });
  }
  $('#wizard').close();
  alert('Setup saved. Connect Google, then run the RSA job search.');
  load();
};

api('/api/health').then(h => {
  document.title = `Tumelo Job Agent ${h.version}`;
